...
static void cashier(void *metadata_pointer) {
  ...
  void *order_list =
      ((void **) (((METADATA *) metadata_pointer)->list_of_order_list))[cashier_index];
  int num_order_cashier =
      ((int *) (((METADATA *) metadata_pointer)->list_of_num_orders))[cashier_index];

  if (num_cashier_created == num_files_cashiers) {
    // printf("------------------------------------------thread_signal: all_cashiers_created\n");
    thread_signal(lock_, all_cashiers_created);
  }

  while (order_index < num_order_cashier) {
    if (order_index == 0 || num_cashier_alive > max_orders) {
      // printf("---------------cashier %d waiting-----------------\n", cashier_index);
      thread_wait(lock_, cashier_post_orders);
    }

    if (board_is_not_full() && posted_order_ready[cashier_index]) {

      cork_board_orders[num_orders_on_cork_board] =
          (int) ((int *) order_list)[order_index];
      cork_board_cashier[num_orders_on_cork_board] = cashier_index;

      cout << "POSTED: cashier " << cashier_index << " sandwich "
           << cork_board_orders[num_orders_on_cork_board] << endl;
      ...
      if (board_is_not_full()) {
        thread_signal(lock_, cashier_post_orders);
      } else {
        thread_signal(lock_, cork_board_full);
      }

      thread_wait(lock_, cashier_index);
    }
  }
...
static void sandwitch_maker(void *metadata_pointer) {
  ...
  thread_lock(lock_);
  thread_wait(lock_, all_cashiers_created);
  ...
  while (true) {
    // printf("-------------------------------------dddddd-----------------\n");
    thread_wait(lock_, cork_board_full);
    ...
    if (num_orders_on_cork_board > 1) {
      for (int i = 1; i < num_orders_on_cork_board; i++) {
        if (abs(cork_board_orders[i], last_sandwitch)
            < abs(cork_board_orders[target_sandwitch_index], last_sandwitch)) {
          target_sandwitch_index = i;
        }
      }
    }

    cout << "READY: cashier " << cork_board_cashier[target_sandwitch_index]
         << " sandwich " << cork_board_orders[target_sandwitch_index] << endl;

    last_sandwitch = cork_board_orders[target_sandwitch_index];

    posted_order_ready[cork_board_cashier[target_sandwitch_index]] = true;
    thread_signal(lock_, cork_board_cashier[target_sandwitch_index]);
...
int main(int argc, char *argv[]) {
  ...
  for (int i = 0; i < argc - 2; i++) {
    ...
    int *orders_per_file =
        (int *) malloc((list_of_num_orders[i] + 1) * sizeof(int));
    std::ifstream file(argv[i + 2]);
    int num_orders_per_file = 0;
  ...
  if (thread_libinit((thread_startfunc_t) all_thread_creation,
                     (void *) metadata_pointer)) {
