...
public class RaftResponses {
    ...
    public static void setTerm (int term) {mTerm = term;}
    
    // @param term for the election.
    // @return null if the internal term not equal to the paramter. array of voting results otherwise.
    public static int[] getVotes (int term) {
        if (term == mTerm) {return mVotes;}
    ...
    public static boolean clearVotes (int term) {
        if (term == mTerm) {
            for (int i=0; i<mVotes.length; i++) {mVotes[i] = -1;}
    ...
    public static int[] getAppendResponses (int term) {
        if (term == mTerm) {return mAppendResponses;}
    ...
    public static boolean clearAppendResponses (int term) {
        if (term == mTerm) {
            for (int i=0; i<mAppendResponses.length; i++) {mAppendResponses[i] = -1;}
    ...
    public static boolean setAppendResponse (int serverID, int response, int term) {
        if (term == mTerm) {
