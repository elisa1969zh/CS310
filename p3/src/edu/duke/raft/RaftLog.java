...
public class RaftLog {
    ...
    public RaftLog (String file) {
        ...
        try {
            ...
            String delims = " ";
            List<String> lines = Files.readAllLines (mLogPath,StandardCharsets.US_ASCII);
            Entry e = null;
            ...
            for (String line : lines) {
                ...
                if ((tokens != null) && (tokens.length > 0)) {
                    e = new Entry (Integer.parseInt (tokens[1]),Integer.parseInt (tokens[0]));
                    mEntries.add (e);
                }
                else {System.out.println ("Error parsing log line: " + line);}
            }
        }
        catch (IOException e) {
    ...
    public int append (Entry[] entries) {
        try {
            if (entries != null) {
                OutputStream out = Files.newOutputStream (mLogPath, StandardOpenOption.CREATE, StandardOpenOption.APPEND, StandardOpenOption.SYNC);
    ...
    public int insert (Entry[] entries, int prevIndex, int prevTerm) {
        if (entries == null) {
        ...
        else if ((prevIndex == -1) || ((mEntries.size () > prevIndex) && (mEntries.get (prevIndex) != null) && (mEntries.get (prevIndex).term == prevTerm))) {
            ...
            try {
                ...
                OutputStream out =
                    Files.newOutputStream (tmpLogPath, StandardOpenOption.CREATE, StandardOpenOption.TRUNCATE_EXISTING, StandardOpenOption.SYNC);
            ...
            try {
                Files.move (tmpLogPath, mLogPath, StandardCopyOption.REPLACE_EXISTING, StandardCopyOption.ATOMIC_MOVE);
            }
            catch (IOException e) {
    ...
    public int getLastIndex () {return (mEntries.size () - 1);}
    
    // @return term of last entry in log
    public int getLastTerm () {
        Entry entry = mEntries.getLast ();
        
        if (entry != null) {return entry.term;}
    ...
    public Entry getEntry (int index) {
        if ((index > -1) && (index < mEntries.size())) {return new Entry (mEntries.get (index));}
    ...
    public String toString () {
        ...
        for (Entry e: mEntries) {toReturn += " (" + e + ") ";}
