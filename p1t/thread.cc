...
static void print_all_three_queues_and_info (void) {
    ...
    for (int i = 0; i< READY_QUEUE.size(); i++) {printf("%p ", READY_QUEUE[i]);}
    ...
    for (std::map<unsigned int, deque<ucontext_t*> >::iterator i = MAP_OF_LOCK_QUEUE.begin(); i != MAP_OF_LOCK_QUEUE.end(); i++) {
        std::cout << i->first << " => [";
    ...
    for (std::map<pair<unsigned int, unsigned int>, deque<ucontext_t*> >::iterator i = MAP_OF_WAIT_QUEUE.begin(); i != MAP_OF_WAIT_QUEUE.end(); i++) {
    ...
    for (std::map<unsigned int, ucontext_t*>::iterator i = MAP_OF_LOCK_OWNER.begin(); i != MAP_OF_LOCK_OWNER.end(); i++) {
...
static int EXIT_HANDLER_THREAD_FUNC (void) {
    while (true) {
        if (THREAD_TO_DELETE != NULL) {
            if (THREAD_TO_DELETE->uc_stack.ss_sp != NULL) {delete (char*) THREAD_TO_DELETE->uc_stack.ss_sp;}
        ...
        if (swapcontext(EXIT_HANDLER_THREAD, THREAD_TO_SWAP_AFTER_DELETE) == -1) {return -1;}
    }
...
static int SWITCH(bool THREAD_EXIT_OR_NOT) {
    ...
    if (!READY_QUEUE.empty()) {                                                          // if (got thread)
        ...
        if (THREAD_EXIT_OR_NOT == true) {
            ...
            if (swapcontext(OLD_THREAD, NEXT_THREAD) == -1) {return -1;}                // swap context; (save my context;  load saved context for thread; )
        }
    }
...
int thread_libinit(thread_startfunc_t func, void *arg) {
    ...
    if (thread_libinit_called > 1) {INTERRUPT_ENABLE(); return -1;}
    
    // LIBINIT_THREAD //
    try {LIBINIT_THREAD = new ucontext_t();}
    
    catch (std::bad_alloc& ba) {INTERRUPT_ENABLE(); return -1;}
    
    if (LIBINIT_THREAD == NULL) {INTERRUPT_ENABLE(); return -1;}
    
    if (context_creation(LIBINIT_THREAD) == -1) {INTERRUPT_ENABLE(); return -1;}
    ...
    if (context_creation(EXIT_HANDLER_THREAD) == -1) {INTERRUPT_ENABLE(); return -1;}
    
    makecontext(EXIT_HANDLER_THREAD, (void(*)()) EXIT_HANDLER_THREAD_FUNC, 0);
    
    if (EXIT_HANDLER_THREAD == NULL) {INTERRUPT_ENABLE(); return -1;}
...
int thread_create(thread_startfunc_t func, void *arg) {
    ...
    if (thread_libinit_called == 0) {INTERRUPT_ENABLE(); return -1;}
    
    try {
        ucontext_t *thread = new ucontext_t();
        
        if (context_creation(thread) == -1) {INTERRUPT_ENABLE(); return -1;}
        
        makecontext(thread, (void(*)()) STUB, 2, func, arg);
        
        READY_QUEUE.push_back(thread);                                                                  // Add thread to ready queue
        
        if (thread == NULL) {INTERRUPT_ENABLE(); return -1;}
    }
...
int thread_yield(void) {
    ...
    if (SWITCH(false) == -1) {INTERRUPT_ENABLE(); return -1;}
...
int thread_lock(unsigned int lock) {
    ...
    if ((thread_libinit_called == 0) || (MAP_OF_LOCK_OWNER[lock] == CURRENT_RUNNING_THREAD) ) {INTERRUPT_ENABLE();return -1;}
    ...
    while ((MAP_OF_LOCK_OWNER[lock] != NULL) && (MAP_OF_LOCK_OWNER[lock] != CURRENT_RUNNING_THREAD)) {  // while (this monitor is not free) {
...
int thread_unlock(unsigned int lock) {
    ...
    if ((thread_libinit_called == 0) || (MAP_OF_LOCK_OWNER[lock] != CURRENT_RUNNING_THREAD)  || MAP_OF_LOCK_OWNER.count(lock) == 0) {INTERRUPT_ENABLE(); return -1;}
...
int thread_wait(unsigned int lock, unsigned int cond) {
    ...
    if (thread_unlock(lock) == -1) {INTERRUPT_ENABLE(); return -1;}                                             // unlock();
    ...
    IS_BLOCKED[CURRENT_RUNNING_THREAD] = true;               // unblock the thread                   // put my TCB on this monitor wait list;
    
    if (SWITCH(false) == -1) {INTERRUPT_ENABLE(); return -1;}
    
    if (thread_lock(lock) == -1) {INTERRUPT_ENABLE(); return -1;}                                               // lock();
...
int thread_signal(unsigned int lock, unsigned int cond) {
    ...
    if (!MAP_OF_WAIT_QUEUE[LOCK_COND_PAIR].empty()) {
        ...
        IS_BLOCKED[MAP_OF_WAIT_QUEUE[LOCK_COND_PAIR].front()] = false;               // unblock the thread
...
int thread_broadcast(unsigned int lock, unsigned int cond) {
    ...
    while (!MAP_OF_WAIT_QUEUE[LOCK_COND_PAIR].empty()) {                                     // repeat thread_signal for however many threads on wait queue
        if (thread_signal(lock, cond) == -1) {INTERRUPT_ENABLE(); return -1;}
    }
